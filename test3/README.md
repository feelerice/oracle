...
1. 使用system用户给lp_user用户分配表空间
   ...
   ```sql
   ALTER USER lp_user QUOTA UNLIMITED ON USERS02;
   ALTER USER lp_user QUOTA UNLIMITED ON USERS;
   ALTER USER lp_user QUOTA UNLIMITED ON USERS03;
   ```

   

2. 在lp_user下创建表，并插入数据
...
通过这次实验：我知道了如果在分区表里查询数据同一个分区查找比不同分区查找快，并且在orders数据量为10000,order_details数据量为30000时，有分区比无分区查找数据优势更大，且分区表在数据量大时进行查找的优势比较大，数据较小时有无分区的查询速度差距不大